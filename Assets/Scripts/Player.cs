	...
	public class Player : MovingObject
	{
		...
		private void OnTriggerEnter2D (Collider2D other)
		{
			...
			else if(other.tag == "First Aid")
			{
				...
				//Call the RandomizeSfx function of SoundManager and pass in two eating sounds to choose between to play the eating sound effect.
				SoundManager.instance.RandomizeSfx (healSound1);

				//Disable the food object the player collided with.
				other.gameObject.SetActive (false);
			}
			else if(other.tag == "Bandage")
			{
				//Add pointsPerFood to the players current food total.
				health += pointsPerBandage;
				...
				//Update foodText to represent current total and notify player that they gained points
				HPText.text = "+" + pointsPerBandage + " health: " + health;
		...
		public void LoseHP (int loss)
		{
			...
			HPText.text = "-"+ loss + " health: " + health;
		...
		{
			//Check if food or health point total is less than or equal to zero.
			if (food <= 0 || health <= 0) 
			{
